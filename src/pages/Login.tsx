...
function Login() {
  ...
  return (
    <div
      style={{
        minHeight: "400vh",
        background: "linear-gradient(to top,rgb(108, 102, 90),rgb(200, 196, 177))",
        display: "flex",
    ...
    >
      ...
      >
        <button onClick={prevSlide} style={{ fontSize: "2rem", background: "none", border: "none", color: "#fff", cursor: "pointer", marginRight: "16px" }}>&lt;</button>
        <img
          src={team[current].img}
          alt={team[current].name}
          style={{ width: "120px", height: "120px", borderRadius: "50%", objectFit: "cover", marginRight: "32px", border: "3px solid #fff" }}
        />
        <div>
          <h2 style={{ margin: 0, color: "#fff" }}>{team[current].name}</h2>
          <h4 style={{ margin: "4px 0", color: "#ffe082" }}>{team[current].role}</h4>
          <p style={{ color: "#f5f5f5", maxWidth: "220px" }}>{team[current].desc}</p>
        </div>
        <button onClick={nextSlide} style={{ fontSize: "2rem", background: "none", border: "none", color: "#fff", cursor: "pointer", marginLeft: "16px" }}>&gt;</button>
      </div>
      <p
          style={{
            position: "absolute",
